...
module WikiGraphvizHelper
	...
	def	create_image_using_dot(layout, fmt, dot_text, result, temps)
		...
		system("/usr/bin/dot -K#{layout} -T#{fmt[:type]} < \"#{temps[:dot].path}\" > \"#{temps[:img].path}\" 2>\"#{temps[:err].path}\"")
		...
		system("/usr/bin/dot -K#{layout} -Timap < \"#{temps[:dot].path}\" > \"#{temps[:map].path}\" 2>\"#{temps[:err].path}\"")
		if !$?.exited? || $?.exitstatus != 0
